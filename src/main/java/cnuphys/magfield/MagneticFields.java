...
public class MagneticFields {
	...
	private Solenoid readSolenoid(String fullPath) {
        ...
        // read the solenoidal field
	private Solenoid readSolenoid(InputStream ins, String filename) {
		...
		Solenoid solenoid = null;
                try {
                    solenoid = Solenoid.fromBinaryFile(ins,filename);
                } catch (FileNotFoundException ex) {
                    Logger.getLogger(MagneticFields.class.getName()).log(Level.SEVERE, null, ex);
                }
		_solenoidPath = filename;
	...
	private Torus readTorus(InputStream ins, String filename) {
...
//			System.exit(1);
		...
		Torus torus = null;
            ...
            try {
                torus = Torus.fromBinaryFile(ins,filename);
            } catch (FileNotFoundException ex) {
                Logger.getLogger(MagneticFields.class.getName()).log(Level.SEVERE, null, ex);
            }
...
//			System.exit(1);
	 ...
	 *             be found
	 */
	public void initializeMagneticFieldsFromFile(InputStream in_torus, InputStream in_solenoid, 
                String filetorus, String filesolenoid)
			throws MagneticFieldInitializationException, FileNotFoundException {

		if ((in_torus == null) && (in_solenoid == null)) {
			throw new MagneticFieldInitializationException();
		}
		...
		if (in_torus != null) {
			System.out.println("  TORUS: [" + filetorus + "]");
			_torus = readTorus(in_torus,filetorus);
		}

		if (in_solenoid != null) {
			System.out.println("  SOLENOID: [" + filesolenoid + "]");
			// load the solenoid
			_solenoid = readSolenoid(in_solenoid,filesolenoid);
		}
		...
		_torusPath = filetorus;
		_solenoidPath = filesolenoid;
